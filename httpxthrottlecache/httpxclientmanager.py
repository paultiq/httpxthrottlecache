...
class HttpxThrottleCache:
    ...
    def __post_init__(self):
        ...
        if self.rate_limiter_enabled and self.rate_limiter is None:
            self.rate_limiter = create_rate_limiter(requests_per_second=self.request_per_sec_limit)

        if (self.cache_mode != "Disabled") and not self.cache_rules:
        ...
        if self._client is None:
            with self.lock:
                ...
                if self._client is None:
                    ...
                    self._normalize_timeout(params)
